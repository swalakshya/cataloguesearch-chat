...
export function createServer(options = {}) {
  ...
  const defaultResponseFormat = normalizeResponseFormat(
    options.defaultResponseFormat ?? process.env.DEFAULT_ANSWER_FORMAT,
    { fallback: "structured", defaultResponseFormat: "structured" }
  ) || "structured";
  ...
  if (testMode) {
    ...
    app.post("/v1/test/shutdown", (req, res) => {
  ...
  function resolveSessionTokenLimit({ explicitLimit, defaultProvider: provider, defaultModel: model }) {
    ...
    const configured = getSessionTokenLimit(provider, model);
    if (Number.isFinite(configured) && configured > 0) return configured;
    log.warn("session_token_limit_unset", { provider, model });
    return 0;
  }
  ...
  function createStageEmitter(onStage) {
    ...
    return (stage) => {
      ...
      if (!key || emitted.has(key) || !chatProgressStages[key]) return;
      emitted.add(key);
      onStage({ stage: key, label: chatProgressStages[key] });
    };
  }
  ...
  async function handleMessageWithProvider({
    ...
    try {
      await compactSessionHistoryIfNeeded({ provider, session, requestId, persist: true });
    } catch (err) {
      log.warn("history_summary_preflight_failed", {
        requestId,
        message: err?.message || String(err),
      });
    }
    ...
    const emptyTextCount = cleanedChunks.filter((chunk) => !String(chunk?.t || "").trim()).length;
  ...
  async function compactSessionHistoryIfNeeded({ provider, session, requestId, persist = false }) {
    const threshold = getHistorySummaryThreshold();
  ...
  function normalizeResponseFormat(value, { fallback = defaultResponseFormat, defaultResponseFormat: defaultFormat } = {}) {
    const effectiveFallback = defaultFormat ?? fallback;
    const raw = value === undefined || value === null || String(value).trim() === "" ? effectiveFallback : value;
...
function mergeFilters(modelFilters, uiFilters) {
  ...
  return {
    ...(modelFilters || {}),
    ...Object.fromEntries(
      Object.entries(uiFilters).filter(([, value]) => value !== undefined && value !== null && value !== "")
    ),
  };
}
